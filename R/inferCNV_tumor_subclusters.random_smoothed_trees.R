...
.single_tumor_subclustering_smoothed_tree <- function(tumor_name, tumor_group_idx, tumor_expr_data, p_val, hclust_method, window_size,
    ...
    hc <- hclust(parallelDist(t(sm_tumor_expr_data), method="cosine", threads=infercnv.env$GLOBAL_NUM_THREADS), method=hclust_method)
...
.parameterize_random_cluster_heights_smoothed_trees <- function(expr_matrix, hclust_method, window_size, plot=FALSE) {
    ...
    d = parallelDist(t(sm_expr_data), method="cosine", threads=infercnv.env$GLOBAL_NUM_THREADS)
    ...
    max_rand_heights <- foreach (i=seq_len(num_rand_iters)) %dopar% {
        ...
        rand.dist = parallelDist(t(sm.rand.tumor.expr.data), method="cosine", threads=infercnv.env$GLOBAL_NUM_THREADS)
